...
```plaintext
  step()
    │
    ├─► FETCH     mem.readWord(PC)         →  raw 32-bit word
    ├─► DECODE    Decoder::decode(raw)     →  opcode / rd / rs1 / rs2 / funct3 / funct7 / imm
    ├─► READ      regs.get(rs1/rs2)
    ├─► EXECUTE   ALU / branch / jal / csr / ecall / amo
    ├─► MEMORY    mem.readWord / mem.write  (load/store)
    └─► WRITEBACK regs.set(rd, result);  PC += 4  (или branch/jal target)
```
...
```plaintext
        ┌─────────────────────────────────────────┐
        │               M-mode                    │
        │  полный доступ ко всем CSR              │
        └──────┬──────────────────────────────────┘
               │ mret (MPP=01)       ▲ ecall / exception / interrupt
               ▼                    │
        ┌──────────────────────────────────────────┐
        │               S-mode                    │
        │  нет доступа к m-CSR                    │
        └──────┬──────────────────────────────────┘
               │ mret (MPP=00)       ▲ ecall / exception
               ▼                    │
        ┌──────────────────────────────────────────┐
        │               U-mode                    │
        │  только cycle/time/instret              │
        │  PTE.U=0 → page fault                   │
        └─────────────────────────────────────────┘
  В XorOS сейчас: только M-mode. S и U — в дорожной карте.
```

Поток таймерного прерывания:

```plaintext
mtime достигает mtimecmp (проверка на каждом step())
  → triggerInterrupt(INT_TIMER_M)
      → fireTrap: mepc=PC, mcause=0x80000007, MIE→MPIE, PC=mtvec
          → следующий step() попадает в trap_entry  (trap.S в os/)
              → trap_handler → int_table[7] = clint_handler
                  → clint_handler записывает новый mtimecmp
                      → on_write-лямбда обновляет mtimecmp в симуляторе
                  → mret: PC=mepc, MPIE→MIE восстановлен
```

Общая схема трапа (ecall / исключение / прерывание):

```plaintext
  ecall / исключение / прерывание
              │
              ▼
      fireTrap(cause)
  ┌─────────────────────────┐
  │ mcause  ← cause         │
  │ mepc    ← PC            │
  │ mstatus.MPP ← PrivMode  │
  │ PrivMode    ← M         │
  │ PC          ← mtvec     │
  └─────────────────────────┘
              │
              ▼
       trap_entry (trap.S)       сохранить x1-x31 на стек (-128 байт)
              │
              ▼
       trap_handler(mcause, mepc, mtval, *frame)
       ecall?  → a7 → диспетчер syscall
       int?    → int_table[cause & 0x7F]
              │
              ▼
       mepc += 4  (только для ecall)
       восстановить x1-x31 со стека
              │
              ▼
            mret
  ┌─────────────────────────┐
...
```plaintext
                        READ                      WRITE
                          │                         │
                    ┌─────▼─────┐            ┌──────▼──────┐
                    │  index_   │ HIT        │ backing_    │  write-through:
                    │ (hashmap) ├───────────►│ MemoryModel │  всегда пишем
                    └─────┬─────┘            └──────┬──────┘  в RAM
                          │ MISS                    │
                          ▼                         │ если адрес
              ┌───────────────────────┐             │ есть в кеше —
              │  backing_.readWord()  │             │ обновляем и его
              │  (MemoryModel → RAM)  │             └──────────────────
              └───────────┬───────────┘
                          │
              ┌───────────▼───────────┐
              │  lru_.push_front()    │   кеш полон?
              │  index_[addr] = iter  │   → lru_.pop_back() (выселяем LRU)
              └───────────────────────┘

  Структура строки:   [ tag (addr) | value (32 бит) ]
  Ёмкость:            64 слова = 256 байт
  Политика чтения:    read-allocate  (промах → загружаем в кеш)
  Политика записи:    write-through  (запись → сразу в MemoryModel)
  Алгоритм вытеснения: LRU           (std::list + unordered_map)
```
...
┌──────────┬───────┬───────┬──────┬───────┬────────┐
│  funct7  │  rs2  │  rs1  │  f3  │  rd   │ opcode │  R  ADD SUB AND OR XOR SLL SRL SRA SLT AMO
└──────────┴───────┴───────┴──────┴───────┴────────┘
┌──────────────────┬───────┬──────┬───────┬────────┐
│     imm[11:0]    │  rs1  │  f3  │  rd   │ opcode │  I  ADDI LW JALR CSR ECALL
└──────────────────┴───────┴──────┴───────┴────────┘
┌──────────┬───────┬───────┬──────┬───────┬────────┐
│ imm[11:5]│  rs2  │  rs1  │  f3  │imm[4:0│ opcode │  S  SW SH SB
└──────────┴───────┴───────┴──────┴───────┴────────┘
┌─┬────────┬───────┬───────┬──────┬──────┬─┬───────┐
│12│ 10:5  │  rs2  │  rs1  │  f3  │ 4:1  │11│opcode│  B  BEQ BNE BLT BGE BLTU BGEU
└─┴────────┴───────┴───────┴──────┴──────┴─┴───────┘
┌──────────────────────────────────┬───────┬────────┐
│           imm[31:12]             │  rd   │ opcode │  U  LUI AUIPC
└──────────────────────────────────┴───────┴────────┘
┌─┬──────────┬─┬──────────┬────────┬───────┬────────┐
│20│  10:1   │11│  19:12  │        │  rd   │ opcode │  J  JAL
└─┴──────────┴─┴──────────┴────────┴───────┴────────┘
```

## Трансляция адресов Sv32

```plaintext
  Виртуальный адрес (32 бита):
  ┌────────────┬────────────┬────────────────┐
  │  VPN[1]    │  VPN[0]    │    offset      │
  │  биты 31:22│  биты 21:12│   биты 11:0    │
  └─────┬──────┴──────┬─────┴───────┬────────┘
        │             │             │
        ▼             │             │
  satp → root_pt      │             │
    ...
    PPN  → l0_pt      │             │
                ...
                l0_pt[VPN[0]]─►PTE  │
                  ...
                  PPN               │
                      │             │
                      ▼             ▼
              физический адрес = (PPN << 12) | offset

  В XorOS: VPN[1] всегда 0 → root_pt[0] → l0_pt[1024 записей]
```
