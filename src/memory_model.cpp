...
MemoryModel::MemoryModel(size_t size) : data_(size, 0u), size_(size) {
    assert(size > 0 && "MemoryModel: size must be > 0");
}

void MemoryModel::loadProgram(const std::vector<uint8_t>& program, Addr base) {
    ...
    if (static_cast<size_t>(base) + program.size() > size_) {
        throw std::out_of_range("MemoryModel::loadProgram — program exceeds memory: base=0x" +
                                std::to_string(base) + " size=" + std::to_string(program.size()) +
                                " limit=" + std::to_string(size_));
    }
    std::copy(program.begin(), program.end(), data_.begin() + base);
}

ByteT MemoryModel::readByte(Addr addr) const {
    checkBounds(addr, 1);
    return data_[addr];
}

HalfT MemoryModel::readHalf(Addr addr) const {
    checkBounds(addr, 2);
    return static_cast<HalfT>(static_cast<unsigned>(data_[addr]) |
                              (static_cast<unsigned>(data_[addr + 1]) << 8));
}
WordT MemoryModel::readWord(Addr addr) const {
    checkBounds(addr, 4);
...
void MemoryModel::write(Addr addr, ByteT val) {
    checkBounds(addr, 1);
    data_[addr] = val;
}

void MemoryModel::write(Addr addr, HalfT val) {
    checkBounds(addr, 2);
    data_[addr]     = val & 0xFFu;
    data_[addr + 1] = (val >> 8) & 0xFFu;
}

void MemoryModel::write(Addr addr, WordT val) {
    checkBounds(addr, 4);
    data_[addr]     = val & 0xFFu;
    data_[addr + 1] = (val >> 8) & 0xFFu;
...
void MemoryModel::dump(Addr from, size_t count) const {
...
void MemoryModel::checkBounds(Addr addr, size_t width) const {
