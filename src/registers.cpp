...
Word Register::read(uint8_t idx) const {
    assert(idx < NUM_REGS && "Register::read — index out of range [0,31]");
    return (idx == 0) ? 0u : regs_[idx];  // x0 = hardwired zero
}

void Register::write(uint8_t idx, Word val) {
    assert(idx < NUM_REGS && "Register::write — index out of range [0,31]");
    if (idx != 0)
...
void Register::setPC(Word val) {
    assert((val & 0x3u) == 0 && "Register::setPC — PC must be 4-byte aligned");
    pc_ = val;
}
...
void Register::dump() const {
    ...
    for (int i = 0; i < NUM_REGS; ++i) {
        std::cout << "x" << std::setw(2) << std::setfill('0') << i << " (" << std::setw(4)
                  << std::setfill(' ') << ABI_NAMES[i] << "): "
                  << "0x" << std::hex << std::setw(8) << std::setfill('0') << regs_[i] << "  ("
                  << std::dec << static_cast<SWord>(regs_[i]) << ")\n";
    }
}