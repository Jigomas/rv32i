...
private:
    ...
    struct CacheEntry {
        ...
        bool operator<(const CacheEntry& other) const { return next_use > other.next_use; }
    };
...
template <typename KeyT, typename ValueT>
void IdealCache<KeyT, ValueT>::LoadAccessPattern(KeyT                       key,
                                                 const std::vector<size_t>& access_positions) {
...
void IdealCache<KeyT, ValueT>::Remove() {
    ...
    auto target = std::max_element(data_.begin(), data_.end(), [](const auto& a, const auto& b) {
